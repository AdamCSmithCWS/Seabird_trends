...
weighted = F #change to TRUE if weighted gam desired.
### final decision to go with unweighted version, because the a posterior weighting by colony size takes care of it
...
ymaxpred = 2017-min(spdat$year-1)
...
# X.basis <- t(solve(X_sqrt.OMEGA_allp,t(X_Kp)))

if(weighted != T){
wts = rep(1,length = length(wts))
wtlst2 = rep(1,length = length(wtlst2))
#wtlst = rep(1,length = length(wtlst)) #unweights the counts, but retains the list of colony sizes to weight the total population estimate
}
...
gamfitqp <- jags.model(data = dat,
                       file = paste0("models/GAM model Westcoast quasialt interaction W jagam2.txt"),
                       n.chains = 3)
...
pdf(paste0("time series models/gam/",ss,"Western quasi alt hierarchical jagam",as.character(weighted)," mcmc diagnostic.pdf"))
plot(poster)
dev.off()
...
for (cc in c("regional","regional.sum")){
 ...
 if(cc == "regional"){
  ...
  # 
  # trendcol[which(trendcol$colony == cc),"long-term"] <- round(median(((post/base)^(1/(ymaxpred-ymin))-1)*100),3)
  # trendcol[which(trendcol$colony == cc),"long-term.lci"] <- round(quantile(((post/base)^(1/(ymaxpred-ymin))-1)*100,0.025),3)
  # trendcol[which(trendcol$colony == cc),"long-term.uci"] <- round(quantile(((post/base)^(1/(ymaxpred-ymin))-1)*100,0.975),3)
  # 
...
png(filename = paste0("time series models/gam/",ss,"western jaggam trajectory overplot quasi poisson hierarchical ",as.character(weighted),".png"),
    res = 300,
    height = 8,
    width = 12,
    units = "in") 
...
xplot = sort(unique(indicescol$year))
pdf(file = paste0("time series models/gam/",ss,"western jaggam trajectory colony panels quasi poisson hierarchical ",as.character(weighted),".pdf"),
 #   res = 300,
    height = 8,
    width = 12) 
par(mar = c(3,3,3,1))
...
for(cc in 1:ncolony){
  ...
  plot(1,1,
       xlim = c(min(indicescol$year),2017),
...
png(filename = paste0("time series models/gam/",ss,"western jaggam trajectory raw overplot quasi poisson hierarchical ",as.character(weighted),".png"),
    res = 300,
    height = 8,
    width = 12,
    units = "in") 
...
write.csv(indicescolony,paste0("colony level indices hierarchical",as.character(weighted)," qpoisson.csv"))

write.csv(indicespop,paste0("population level indices hierarchical",as.character(weighted)," qpoisson.csv"))

write.csv(indicescolonyr,paste0("colony level indices hierarchical scaled",as.character(weighted)," qpoisson.csv"))

write.csv(indicespopr,paste0("population level indices hierarchical scaled",as.character(weighted)," qpoisson.csv"))


write.csv(trendcolout,paste0("population level trends hierarchical scaled",as.character(weighted)," qpoisson.csv"))
